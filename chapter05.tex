...
The most straight-forward option is to represent the state of the world the speaker wishes to communicate as a two-dimensional object, where each part of the utterance is evaluated as transmitting one dimension, respectively. 
Another more complex option is to consider a QUD-based model modelling the speaker as communicating reference, predication or both goals, inspired by \textcite{kao2014nonliteral}. In that case, the speaker-utility is defined with respect to a certain QUD she has in mind. The $L_1$ then reasons about the intended goal. 
Yet another possibility is to follow the idea proposed by \textcite{yoon2016talking}, representing the speaker as trying to achieve a weighted combination of two informational goals (reference and predication). 

In line with basic RSA constructions, all these possibilities are cashed out with respect to communicating a certain state of the world most efficiently to a listeral listener $L_0$. To do so, the speaker considers different utterances. As mentioned above, one novel aspect of this model is that the speaker constructs the utterance incrementally: she considers the subject and the predicate separately, choosing uniformly at random where to put the noun. That is, the speaker may put the noun in the subject where the possible noun options are the subordinate or the basic-level label of the intended referent, akin to the experimental subject-N condition. The predicate is then a bare adjective (big or small). The speaker knows which adjective she wants to use. Alternatively, the speaker may put the noun in the predicate, again choosing between the subordinate and basic-level referent label, directly combining it with the adjective; the subject is then the bare deictic 'that' (resulting in the predicate-N experimental condition). 

The $L_0$ then interprets the utterance chosen by $S_1$ according to its literal meaning, considering the subject as establishing reference, and the predicate as communicating the size of the referent. That is, $L_0$ returns the joint distribution over referents from the context of which the subject is true, and properties set by the comparison class. The bare deictic 'that' and the basic-level are assumed to be true of any member of the context, subordinate labels only apply to respective category members. If the predicate contains a noun, that noun is taken as the comparison class, represented by a prior over relative sizes represented by a Gaussian. Is the predicate is a bare adjective, the comparison class variable lifted to the level of $L_1$ passed down provides the comparison class. 
