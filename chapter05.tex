...
\pt{potentially restructure order from L0 to L1??}
...
\end{equation}
That is, the listener infers the referents, its size and the comparison class by reasoning about how a speaker would behave in order to communicate a particular referent and its size, assuming a particular comparison class and context, in addition to his prior knowledge about different subordinate categories and how likely a particular comparison class is to be used. \pt{how a speaker would behave in order to communicate a particular referent and its size in a particular context. Importantly, rather than representing the comparison class as a varibale lifted to the level of L1 \parencite[as proposed by][]{tessler2017warm}, the intended comparison class influencing the resolution of the adjective is represented as a concious \emph{lexical choice} made by the speaker \parencite[following the ideas proposed by][]{chierchia2012grammatical, problang}. } %As in the \textcite{tessler2017warm} model, the listener decides between the subordinate and the superordinate comparison class. 
\pt{discuss how this is computed/scored by the listener}
...
\pt{discuss lexical enrichment / choice, its influences}
...
\pt{think about these bells and whistles; but they would all be built on this same basis}
...
Yet another possibility is to follow the idea proposed by \textcite{yoon2016talking}, representing the speaker as trying to simulataneously achieve a combination of the two informational goals (reference and predication) weighted by a free parameter $\phi$ (Model 3):
\begin{equation}
...
\pt{discuss non-deterministic mapping of pred-N to CC and its power in us not hard-coding anything, and its importance for integrating the cc-variable into these sentences. Threshold inference put down to L0 for easier tractability}
