...
From a semantic perspective, utterances then might have several dimensions of interpretation, each dimension corresponding to the interpretation of the utterance as satisfying a distinct communicative goal, or answering a distinct QUD. This idea was first formlazed within the RSA-framework by \textcite{kao2014nonliteral}, applied to hyperbolic and pragmatic halo effects in interpretation of number words. The crucial novelty they introduced was listener uncertainty about the relevant QUD along with the multi-dimensional literal meaning of utterances where each dimension satisfied a distinct potential QUD. The speaker in this model was thus choosing utterances optimally conveying her particular intended QUD \parencite{kao2014nonliteral}. \pt{add formulas if necessary}

Another approach to formalizing multiple conversational goals was taken by \textcite{yoon2016talking}. The authors proposed a model of polite language, importantly noting that although politeness phenomena were argued to violate basic cooperative principles of quality, they can be rationally explained as  produced by interlocutors pursuing not only the goal of informativeness, but simultaneously also a \emph{social goal} \parencite[cf.][]{brown1987politeness}. That is, by using polite sentences speakers strive to save the listener's self-image, or \emph{face}. Omitting here the details of how exactly the speaker utility for achieving this social goal was formalized, the authors proposed a model wherein the speaker tries to balance the utility of an utterance as simulatneously achieving the social goal and the goal of being maximally informative by maximizing a global utility function, which consisted of a weighted combination of these two utility sub-functions \parencite{yoon2016talking}.  

Yet the majority of RSA models usually assume that speakers address one particular conversational goal. The listener might be uncertain about it, as in case of QUD-models, or the model might just addressed one pre-defined aspect of communication, i.e., one QUD \parencite[see][for an excellent overview of various RSA models]{problang}. This work has argued that interlocutors at least consider multiple informational goals (i.e., QUDs) that might be addressed within the same utterance containing a noun and a gradable adjective, where different parts of the utterance might probabilistically contribute to achieving each of the goals. Therefore, in the following a model is proposed which attempts to formalize the reference-predication hypothesis. It treats speakers' choice of utterances as incremental, where the subject is chosen so as to establich reference, and the predicate is chosen to convey the comparison class --- and so one utterance is optimized to fulfil multiple QUDs. 

The basic structure of the reference-predication RSA-model is built off the model by \textcite{tessler2017warm}. Then, different potential representations of the speaker-utility function integrated in that basic structure are presented and discussed. Finally, model predictions will be qualitatively compared to the results of the Comparison Class Inference Experiment (s. Section \ref{experiment3}). It will be shown that minimal extensions to existing models result in accurate predictions about identity of an unknown referent and its property, but predictions about the lifted comparison-class variables are limited if assuming the representation used by \parencite{tessler2017warm}. 

In the basic model, upon hearing an utterance of the form "That N is ADJ" or "That's a ADJ N" the pragmatic listener tries to resolve his uncertainty about the specific member of the context the speaker is referring to, its size and the intended comparison class. As a first approximation, driven by experimental results and previous work by \textcite{tessler2017warm}, it is assumed that the potential comparison classes are either the subordinate or the basic-level category of that referent. Therefore, the listener is assumed to know the subordinate categories of all the potential referents.
Formally, the listener infers the comparison class $cc$, the referent $r$ and the size $s$ of the referent given an utterane and a perceptual context $C$:

\begin{equation}
P_{L_1} (r, s, cc \mid u, C) \propto P_{S_1} (u \mid s, r, cc, C) \times P(s \mid cc_{r\_sub}) \times P(r \mid C) \times P(cc)
\end{equation}

That is, the listener is both uncertain about the intended referent which might be one of the members of the perceptual context, and about the size of that referent given its subordinate category. The listener infers them by reasoning about how a speaker would behave in order to communicate a particular referent and its size, assuming a particular comparison class and context, in addition to his prior knowledge about different subordinate categories and how likely a particular comparison class is to be used. %As in the \textcite{tessler2017warm} model, the listener decides between the subordinate and the superordinate comparison class. 
...
\end{equation}
The cost of producing uterances $C(u)$ is assumed to be uniform over all possible utterances and can therefore be neglected. Now different options are considered here how to represent the speaker utility. 
The most straight-forward option is to represent the state of the world the speaker wishes to communicate as a two-dimensional object, where each part of the utterance is evaluated as transmitting one dimension, respectively (Model 1). 
\begin{equation}
U_{S_1} (u; r; s; cc; C) = log \: L_0 (s, r \mid u, C, cc) 
\end{equation}
Another more complex option is to consider a QUD-based model modelling the speaker as communicating reference, predication or both goals, inspired by \textcite{kao2014nonliteral}. In that case, the speaker-utility is defined with respect to a certain QUD she has in mind. The $L_1$ then reasons about the intended goal (Model 2).
\begin{equation}
U_{S_1} (u; r; s; cc; C; QUD) = log \: L_0(QUD(s, r) \mid u, C, cc)
\end{equation} 
where $QUD(.)$ projects the state of the world onto the subspace relevant for the particular intended communicative goal. 

Yet another possibility is to follow the idea proposed by \textcite{yoon2016talking}, representing the speaker as trying to achieve a weighted combination of two informational goals (reference and predication) (Model 3):
\begin{equation}
...
In line with basic RSA mechanisms, all these possibilities are grounded in a a listeral listener $L_0$ to whom the speaker optimally communicates a certain state of the world. To do so, the speaker considers different utterances. As mentioned above, one novel aspect of this model is that the speaker constructs the utterance incrementally: she considers the subject and the predicate separately, choosing uniformly at random where to put the noun. That is, the speaker may put the noun in the subject where the possible noun options are the subordinate or the basic-level label of the intended referent, resulting in the experimental subject-N condition. The predicate is then a bare adjective ("big" or "small"). %The speaker knows which adjective she wants to use. 
Alternatively, the speaker may put the noun in the predicate, again choosing between the subordinate and basic-level referent label, directly combining it with the adjective; the subject is then the bare deictic "that" (resulting in the predicate-N experimental condition). 

The $L_0$ then interprets the utterance chosen by $S_1$ according to its literal meaning, considering the subject as establishing reference, and the predicate as communicating the size of the referent. That is, $L_0$ returns the joint distribution over referents from the context of which the subject is true, and over properties set by the comparison class. The bare deictic 'that' and the basic-level are assumed to be true of any member of the context, subordinate labels only apply to respective category members. The referential utility of the subject is therefore formalized as in the vanilla RSA model (s. Section \ref{intro-rsa}). If the predicate contains a noun, that noun is taken as the comparison class, represented by a Gaussian prior over relative sizes. Is the predicate is a bare adjective, the comparison class variable lifted to the level of $L_1$ passed down provides the comparison class (as proposed by \textcite{tessler2017warm}). The inference over the threshold of comparison is moved to the level of $L_0$ for better tractability. 
\begin{equation}
...
\pt{Predictions of the different models / conceptual differences / discussion and comparison to E3}
  
%Basic scaffolding of the model: L0 and meaning function: reference via sentence subject, predication via predicate; that and dog always true; threshold semantics for predication. State prior of L0: ref from context, pred from cc passed (as in Tessler model). 

%Speaker strives to communicate a particular state of the world by choosing the optimal utterance via maximizing the utility function. Different options for that: discuss standard ref-size model / QUD model; mixture model; potentially salience model. 

%Listener L1 respectively infers cc, state of the world (potentially other aspects).    
