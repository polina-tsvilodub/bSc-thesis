...
\end{equation*}
However, based on previous models addressing multiple potential questions under discussion, there are other conceivable representations of the speaker utility \parencite[cf.][]{kao2014nonliteral, yoon2016talking}. These representations could be applied in order to extend the model by \textcite{tessler2017warm} wherein the comparison class is treated as a variable listed to the level of the pragmatic listener. That is, the speaker would choose an optimal utterance to communicate a state given a particular comparison class. 

For instance, another more complex option is to consider a QUD-based model wherein the speaker chooses utterances specifically communicating reference or predication only, or both goals, inspired by \textcite{kao2014nonliteral}. In that case, the speaker-utility is defined with respect to a certain QUD she has in mind: 
\begin{equation*}
...
\begin{equation*}
P_{L_1} (r, s, cc \mid u, C) \propto \sum_{QUD} P_{S_1} (u \mid s, r, cc, C, QUD) \; P(r, s \mid C, cc_{r\_sub}) \; P(cc) \; P(QUD)
\end{equation*} 
