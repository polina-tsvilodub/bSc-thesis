...
``` {r}
# read the data
d_rate <- read_csv('./../data/E1_sentence_rating/results_27_syntax-rating-prereg-80_cleaned.csv')
d_rate_second <- read_csv('./../data/E1_sentence_rating/results_27_syntax-rating-prereg-29_cleaned.csv')
...
#write_csv(myvars, path = "./../writing/R4Tex/myvars.csv", col_names = T)
```
...
``` {r}
# wrangle dataset for exploratory analysis based on excluding by failing warm-ups only 
d_rate_main_fullData <- d_rate_passedWarmup %>% filter((trial_name == "custom_slider1") |
                                      (trial_name == "custom_slider2")) %>%
  select(submission_id, trial_number, trial_name, np, sentence_order, domain, item, target_size, sentence1, sentence2, response1, response2)

# turn NP, syntax, sentence order to factors 
d_rate_main_fullData <- d_rate_main_fullData %>% mutate(NP = factor(np, levels = c(0, 1),
                                                  labels = c("sub", "basic")),
                                      target_size = factor(target_size, levels = c(0, 1),
                                                           labels = c("big", "small")),
                                      sentence1 = ifelse(sentence1 == 0, "predicate", "subject"),
...
d_rate_main_tidy_fullData <- d_rate_main_fullData %>%
  mutate(predicate = ifelse(sentence1 == "predicate", response1, response2),
         subject = ifelse(sentence1 == "subject", response1, response2)) %>% 
  select(-sentence1, -sentence2, -response1, -response2) %>% 
  gather(syntax, response, predicate, subject) %>%
...
# contrasts for full dataset
d_rate_main_tidy_fullData <- d_rate_main_tidy_fullData %>% mutate(syntax = ifelse(syntax =="subject", 1, -1 ),
                                                NP = ifelse( NP == "basic", 1, -1   ),
                                                # predicate | subject is -1 , subject|predictae is 1
                                                order = ifelse(sentence_order == "0|1", -1 , 1),
                                                target_size_contr = ifelse(target_size == "big", 1, -1)
...
``` {r}
# exploratory brm model on full dataset
b.lm.full <- brm(response ~ syntax*NP + 
                  (1 + syntax*NP | submission_id) + 
                  (1 + syntax*NP  | item),
                data = d_rate_main_tidy_fullData,
                family = "gaussian")

summary(b.lm.full) -> b.lm.full.summary

# csv output
write_csv(data.frame(b.lm.full.summary[["fixed"]]) %>% 
            mutate(Rowname = row.names(.)), 
          path = "../writing/R4Tex/expt1_brm_full_exploratory.csv")
