...
``` {r data}
# read data
d_infer <- read_csv('~/Documents/Research/refpred/data/cogsci2020/results_30_prereg-cc-inf.csv')
...
write_csv(data.frame(summary(lm.b.infer.fit.by.target)[["fixed"]]) %>%
        ...
        path = "./../../writing/R_results_TeX/expt3_full_brm_revised2.csv")
```
...
write_csv(data.frame(lm.b.infer.fit.two.way2.sum.summary[["fixed"]]) %>%
            mutate(Rowname = row.names(.)),
          path = "./../../writing/R_results_TeX/expt3_2_way_full_brm_revised2.csv")
```
...
write_csv(
  data.frame(lm.b.infer2way.full.contrasts) %>% 
    mutate(Rowname = key), 
          path = "./../../writing/R_results_TeX/expt3_2way_full_contrasts_wProbs.csv"
)
```


```{r}
# two-way analysis, DUMMY / CONTRAST CODED
...
### MH post-cogsci analysis

#### Subjects who only give basic-level responses
```{r}
d_infer_main_responseCat %>%
  group_by(submission_id) %>%
  summarize(prop_basic = mean(response_num)) %>%
  ungroup() %>%
  group_by(prop_basic) %>%
  count()
...
d_infer_main_responseCat %>%
  ...
  summarize(prop_basic = mean(response_num)) %>%
  filter(prop_basic == 1) %>%
  pull(submission_id) -> ids_all_basic
```
#### By item plot: basic-context, subordinate-NP

```{r}
d_infer_main_responseCat %>%
  #filter(!(submission_id %in% ids_all_basic)) %>%
  filter(NP == "subordinate", context == "basic") %>%
  group_by( context, NP, condition, target_size, item) %>%
  tidyboot_mean(column = response_num) -> d_infer_main_item_bs_basic_subNP
```

```{r}

ggplot(d_infer_main_item_bs_basic_subNP %>%
         ungroup() %>%
         mutate(condition = factor(condition, levels = c("predicative", "prenominal"),
                                   labels = c("Subject NP", "Predicate NP"))), aes( x = condition, y = mean, ymin = ci_lower, ymax = ci_upper))+
  geom_col(position = position_dodge(), color = 'black', alpha = 0.3)+
  geom_linerange(position = position_dodge())+
  facet_grid(item ~ target_size)

ggsave("~/Desktop/expt3_basicContext_subNP_byItem.pdf", width = 6, height =6)
```

### Number of switchers
``` {r}
d_infer_main_responseCat %>% filter((context=='basic') & (NP == 'subordinate')) %>% group_by(submission_id, condition) %>% count() %>% spread(condition, n) %>% filter(!(is.na(predicative) | is.na(prenominal))) %>% summarise(sum = predicative + prenominal) %>% summarize(mean = mean(sum))
#summarize(mean = mean(response_num)) %>% ungroup() %>% group_by(submission_id) %>% spread(condition, mean) %>% filter(!(((predicative == 1) & (prenominal == 1)) | ((predicative == 0 ) & (prenominal == 0)))) %>%
filter(!(is.na(predicative) | is.na(prenominal)))
```

``` {r}
# some general analytics
d_infer_main_responseCat %>% filter(context == "basic") %>% count(condition, pic)

d_infer_main_responseCat %>% filter((context=='basic') & (NP == 'subordinate')) %>% group_by(submission_id) %>% summarise(mean = mean(response_num)) %>% summarise(mean(mean))
```
...
bar.width = 0.8
d_infer_main_responseCat %>%  
  mutate(target = gsub("images/", "", pic),
         target = gsub(".png", "", target)) %>%
  group_by( context, NP, condition, target) %>%
  tidyboot_mean(column = response_num) -> d_infer_main_responseCat.full.byItem


d_infer_main_responseCat.full.byItem %>%
  ungroup() %>%
  mutate(condition = factor(condition, levels = c( "predicative", "prenominal"),
                            labels = c(  "Subj NP", "Pred NP")),
         context = factor(context, levels = c("basic", "subordinate"),
                          labels = c("Basic Context","Subordinate Context")),
         NP = factor(NP, levels = c("basic", "one", "subordinate"),
                     labels = c("basic", '"one"', "subordinate"))) %>%
  ggplot(., aes(x=condition, y = mean, fill = NP, ymin = ci_lower, ymax = ci_upper)) +
  geom_col(position = position_dodge(bar.width), width = bar.width, color= 'black',
  ...
  ggthemes::theme_few()+
  xlab("") +
  theme(legend.position = c(0.88, 0.82), legend.text = element_text(size = 9),
        legend.title = element_text(size = 9), 
        legend.key.size = unit(0.5,"line"))+
  ...
  ylab("Proportion of basic-level responses")+
  ggtitle("Experiment 3: Comparison Class Inference")+
  facet_wrap(context~target, ncol = 6 )  
ggsave('figs/cogsci-e3-full-byItem.png', height = 5, width = 8)
```
...
brm.basic.sub <- brm(response_num ~ syntax_contr +
...
```
``` {r}
# subest by NP across contexts
d_infer_NPsub <- d_infer_main_responseCat %>%
  filter(NP == "subordinate")

brm.NPsub.2cont <- brm( response_num ~ syntax_contr + context_sub + 
                        (1 + syntax_contr + context_sub | submission_id)+
                        (1 + syntax_contr + context_sub | item),
                      data = d_infer_NPsub,
                      family = "bernoulli",
                      cores = 4,
...
brm.sub.sub <- brm(response_num ~ syntax_contr +
                        (1 + syntax_contr | submission_id)+
                        (1 + syntax_contr | item),
                      data = d_infer_contextSub_sub,
                      family = "bernoulli",
...
brm.sub.basic <- brm(response_num ~ syntax_contr +
                        (1 + syntax_contr | submission_id)+
                        (1 + syntax_contr | item),
                      data = d_infer_contextSub_basic,
                      family = "bernoulli",
